...
class RolloutStorage(object):
    """class that contains the agent's experiences."""

    def __init__(self, num_steps, obs_shape, num_actions, num_agents,
                 minibatch, gamma, gae_lambda):
        """init method.
        ...
        self.states = torch.zeros(
            num_steps + 1, num_agents, obs_shape, dtype=torch.float32)
        self.rewards = torch.zeros(
            num_steps, num_agents, 1, dtype=torch.float32)
        self.value_preds = torch.zeros(
            num_steps + 1, num_agents, 1, dtype=torch.float32)
    ...
    def compute_returns(self, next_value, use_gae=True):
        if use_gae:
            self.value_preds[self.step] = next_value
            device = next_value.device
            gae = torch.zeros(self.num_agents).to(device)
            for agent_id in range(self.num_agents):
                for step in reversed(range(self.step)):
                    delta = (self.rewards[step, agent_id] + self.gamma * self.value_preds[step + 1, agent_id]
                             * self.masks[step + 1, agent_id] - self.value_preds[step, agent_id])

                    gae[agent_id] = delta + self.gamma * \
                        self.gae_lambda * self.masks[step +
                                                     1, agent_id] * gae[agent_id]
                    self.returns[step, agent_id] = gae[agent_id] + \
    ...
    def recurrent_generator(self, advantages):
        ...
        for start_ind in range(0, total_samples, self.minibatch):
            states_minibatch = []
            actions_minibatch = []
            log_probs_minibatch = []
            ...
            if start_ind + self.minibatch > total_samples:
                break

            for offset in range(self.minibatch):
