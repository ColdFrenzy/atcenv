...
from atcenv.utils.wind_utils import abs_compass
from atcenv.utils.definitions import *
from atcenv.envs.env_utils import speed_penalty, target_dist, target_reached
...
class FlightEnv(MultiAgentEnv):
    ...
    def __init__(self,
                 ...
                 # [-5.0, 0.0, 5.0]
                 accelleration: Optional[List[float]] = [-2.0, 0.0, 2.0],
                 yaw_angles: Optional[List[float]] = [-3.0,
                                                      0.0, 3.0],  # [-5.0, 0.0, 5.0]
    ...
    def resolution(self, actions: Dict) -> None:
        ...
        for f_id, action in actions.items():
            ...
            # TODO discontinuity here when the track angle is closer to 2PI
    ...
    def reward(self) -> Dict:
        ...
        for f_id, flight in self.flights.items():
            ...
            if self.reward_as_dict:
                ...
                drift_rew = min_max_normalizer(flight.drift, 0, 2*math.pi)
    ...
    def observation(self) -> Dict:
        ...
        for i, flight in self.flights.items():
            ...
            # bearing angle between [0, 2PI] normalized between 0 and 1
            b = min_max_normalizer(flight.bearing, 0, 2 * math.pi)
            # tracking angle between [0, 2PI] normalized between 0 and 1
            t = min_max_normalizer(flight.track, 0, 2*math.pi)
    ...
    def polar_distance(self, f: Flight):
        """
        Computes the normalized polar distance between a flight and all the other flights in the current field of view
        :param flight: Flight object
        :return left_angle: angle of an object in the left side of the FoV.
            This value is 0.0 if no object is seen, otherwise it's between [1., 1.5]
            1 at the extreme and 1.5 near the center of the FoV
        :return right_angle: angle of an object in the right side of the FoV.
            This value is 0.0 if no object is seen, otherwise it's between [1., 1.5]
            1 at the extreme and 1.5 near the center of the FoV
        :return dist: normalized distance of the object inside the FoV
        """
        ...
        origin = f.position
        seen_agents_indices = self.flights_in_fov(f.flight_id)
        if len(seen_agents_indices) != 0:
            ...
            if len(seen_agents_indices) <= self.max_agent_seen:
                for j, seen_agent_idx in enumerate(seen_agents_indices):
                    ...
                    # angle is normalized between 0 and 1, in zero we are at the beginning
                    # of the fov (clockwise) at 1 we are at the end of the fov
                    angles[j] = min_max_normalizer(angle, 0, f.fov_angle)
                    # with the following condition, we map the FOV angle such that it is 1 on the extremes
                    # and 1.5 in the middle
                    if angles[j] >= 0.5:
                    ...
                    dists[j] = 1. - math.dist([self.flights[seen_agent_idx].position.x,
                ...
                # take the max_agent_seen closest agent
                for j in range(self.max_agent_seen):
                    ...
                    assert 0.0 <= angles[
                        j] <= 1.0, f"The FoV angle is invalid: {angles[j]}. The angle should be between [0,1]"
                    dists[j] = 1 - math.dist([nearest_agent.x,
                                              nearest_agent.y],
                                             [origin.x, origin.y]) / f.fov_depth
