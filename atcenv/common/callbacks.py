...
class QmixCallbacks(DefaultCallbacks):

    def __init__(self):
        super(QmixCallbacks, self).__init__()
    ...
    def on_episode_step(self,
        ...
        env = base_env.envs[0]
        self.num_conflicts += len(env.env.conflicts)
    ...
    def on_episode_end(self, *, worker: "RolloutWorker", base_env: BaseEnv,
        ...
        env = base_env.envs[0]
        flights = list(env.env.flights.values())
        ...
        episode.custom_metrics["speed_diff"] = float(
            np.asarray(speed_diff).mean())
        episode.custom_metrics["actions_accel"] = np.asarray(
            env.env.logging_actions['accel']).mean()
        episode.custom_metrics["actions_track"] = float(
            np.bincount(env.env.logging_actions['track']).argmax())
        episode.custom_metrics["non_zero_obs"] = float(
            np.asarray(env.env.logging_obs['non_zero']).mean())
        #episode.hist_data["actions_track"] = env.logging_actions['track']

        # get all the agents that reached the target
        done_ids = [v for k, v in env.env.done.items()
                    if v and k != "__all__"]
        done_ids = len(done_ids) / len(env.env.flights)
        episode.custom_metrics["reached_target"] = done_ids
